...
// Build apps list menu
  ...
  foreach (functions::admin_get_apps() as $app) {
    if (!k::current_user_can('load-app:' . $app['code'])) continue;
    ...
    if (!empty($app['menu'])) {        
      foreach ($app['menu'] as $item) {
        if($app['code']=='catalog'){
          if (!settings::get('product_groups_setting') && $item['doc']=='product_groups') continue; 
          if (!settings::get('option_groups_setting') && $item['doc']=='option_groups') continue; 
          if (!settings::get('manufacturers_setting') && $item['doc']=='manufacturers') continue; 
          if (!settings::get('suppliers_setting') && $item['doc']=='suppliers') continue; 
        }

        if($app['code']=='settings'){
            if($item['doc']=='site') continue; 
        }
