...
<?php
    ...
    for ($timestamp = strtotime('-11 months'); date('Y-m', $timestamp) <= date('Y-m'); $timestamp = strtotime('+1 month', $timestamp)) {
      ...
      //Spanish language
    if(language::$selected['code']=='es')
      $month=substr(language::strftime_es(language::strftime('%B', $timestamp)),0,3);
    else
      $month=language::strftime('%b', $timestamp);

      $monthly_sales[date('Y-m', $timestamp)] = '[\''. $month .'\', '. (int)$orders['total_sales'] .']';
    }
?>
