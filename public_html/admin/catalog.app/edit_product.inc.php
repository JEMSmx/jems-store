<?php
  ...
  if (isset($_POST['save'])) {
    ...
    if (empty(notices::$data['errors'])) {
      ...
      if (!isset($_POST['videos'])) $_POST['videos'] = array();
      if (!isset($_POST['showcases'])) $_POST['showcases'] = array();
      ...
      foreach (array_keys($_POST['showcases']) as $key) {
           foreach(array_keys($_POST['showcases'][$key]['showcase_columns']) as $column_key){ 
             if (!empty($_FILES['showcases']['tmp_name'][$key]['showcase_columns'][$column_key]['new_images_showcase'])) {
                 $product->add_image_showcase_row($_FILES['showcases']['tmp_name'][$key]['showcase_columns'][$column_key]['new_images_showcase'],$key,$column_key);
             }
           }
      }
      
      foreach (array_keys($_POST['delete-archive']) as $key) {
       if(!empty($_POST['delete-archive'][$key]))
        $product->delete_image($_POST['delete-archive'][$key]);
      }
      ...
      if (!empty($_POST['new_videos'])) {
        foreach (array_keys($_POST['new_videos']) as $key) {
        if(empty($_POST['new_videos'][$key])) continue;
          $product->add_video($_POST['new_videos'][$key]);
        }
      }

      $product->save();
      if(isset($_POST['redirect']) && !empty($_POST['redirect'])){
        notices::add('success', language::translate('success_changes_saved_showcase', 'Changes saved'));
        header('Location: '. document::link('', array('app' => $_GET['app'], 'doc' => 'edit_product', 'product_id' => $product->data['id'], 'showcase' => 'active')));
        exit;
      }else{
...
<?php echo functions::form_draw_form_begin(false, 'post', false, true, 'id="catalogProduct"'); ?>

  <div class="tabs">

    <ul class="index">
      <li id="general"><a href="#tab-general"><?php echo language::translate('title_general', 'General'); ?></a></li>
      ...
      <?php if (settings::get('option_groups_setting')) { ?> 
        <li><a href="#tab-options"><?php echo language::translate('title_options', 'Options'); ?></a></li>
      <?php } ?> 
       <?php if (settings::get('product_groups_setting')){ ?> 
      <li><a href="#tab-options-stock"><?php echo language::translate('title_options_stock', 'Options Stock'); ?></a></li>
      <?php } ?> 
      <li id="showcase"><a href="#tab-showcase"><?php echo language::translate('title_showcase', 'Showcase'); ?></a></li>
    </ul>
              ...
              <script>
                $("body").on("click", "#add-new-image", function(event) {
                  ...
                  $(this).closest("table").find("tr:last-child").before('<tr><td><?php echo str_replace(array("\r", "\n"), '', functions::form_draw_file_field('new_images[]', 'data-size="large"')); ?></td></tr>');
                     ...
                     <?php if (!empty($product->data['videos'])) { ?>
          <tr>
            <td><strong><?php echo language::translate('title_product_videos', 'Product Videos'); ?></strong><br />
...
function youtube_id_from_url($url) {
  ...
  if (!empty($_POST['videos'])) foreach (array_keys($_POST['videos']) as $key) {
?>
    <tr>
      <?php echo functions::form_draw_hidden_field('videos['.$key.'][id]', true); ?>
      <td><img height="75" width="100" src="https://img.youtube.com/vi/<?php echo youtube_id_from_url($_POST['videos'][$key]['link']); ?>/mqdefault.jpg" align="left" style="margin: 5px;" /></td>
      <td><?php echo functions::form_draw_hidden_field('videos['.$key.'][link]', $_POST['videos'][$key]['link']); ?><?php echo functions::form_draw_text_field('videos['.$key.'][new_link]', isset($_POST['videos'][$key]['new_link']) ? $_POST['videos'][$key]['new_link'] : $_POST['videos'][$key]['link'], 'data-size="large"'); ?></td>
      <td><a class="move-up" href="#" title="<?php echo language::translate('text_move_up', 'Move up'); ?>"><?php echo functions::draw_fonticon('fa-arrow-circle-up fa-lg', 'style="color: #3399cc;"'); ?></a> <a class="move-down" href="#" title="<?php echo language::translate('text_move_down', 'Move down'); ?>"><?php echo functions::draw_fonticon('fa-arrow-circle-down fa-lg', 'style="color: #3399cc;"'); ?></a> <a class="remove" href="#" title="<?php echo language::translate('title_remove', 'Remove'); ?>"><?php echo functions::draw_fonticon('fa-times-circle fa-lg', 'style="color: #cc3333;"'); ?></a></td>
    </tr>
<?php
  }
?>
              ...
              <script>
                $("#table-videos").on("click", ".move-up, .move-down", function(event) {
                  event.preventDefault();
                  var row = $(this).closest("tr");
                ...
                $("#table-videos").on("click", ".remove", function(event) {
                  event.preventDefault();
                  $(this).closest('tr').remove();
                });
              </script>
     ...
     <?php } ?>
          <tr>
            <td><strong><?php echo language::translate('title_add_video', 'Add Video'); ?></strong><br />
              <table>
                <tr>
                  <td><?php echo functions::form_draw_text_field('new_videos[]', true, 'data-size="large"'); ?></td>
                </tr>
                <tr>
                  <td><a href="#" id="add-new-video" title="<?php echo language::translate('text_add', 'Add'); ?>"><?php echo functions::draw_fonticon('fa-plus-circle', 'style="color: #66cc66;"'); ?></a></td>
                </tr>
              </table>
              <script>
                $("body").on("click", "#add-new-video", function(event) {
                  event.preventDefault();
                  $(this).closest("table").find("tr:last-child").before('<tr><td><?php echo str_replace(array("\r", "\n"), '', functions::form_draw_text_field('new_videos[]', true, 'data-size="large"')); ?></td></tr>');
                });
              </script>
              </td>
          </tr>           
      ...
      <div id="tab-information">
        <table>
         <?php if (settings::get('manufacturers_setting')) { ?>  
          <tr>
            <td>
          ...
          <?php } ?>
          <?php if (settings::get('suppliers_setting')) { ?> 
          <tr>
            <td>
...
?>
          ...
          <tr>
            <td><h3><?php echo language::translate('title_seo', 'SEO'); ?></h3></td>
          </tr>
...
?>
        ...
        <?php $options_alignment = array(array(language::translate('showcase_column_choose_option', 'Choose'),"center"),array(language::translate('showcase_column_alignment_left', 'Left'),"left"),array(language::translate('showcase_column_alignment_center', 'Center'),"center"),array(language::translate('showcase_column_alignment_right', 'Right'),"right"));

        $options_scheme = array(array(language::translate('showcase_column_choose_option', 'Choose'),"center"),array(language::translate('showcase_column_scheme_dark', 'Dark'),"obscuro"),array(language::translate('showcase_column_scheme_light', 'Light'),"Claro"));?>
      <div id="tab-showcase">
      <h1>Showcase</h1>
          <table id="table-showcase-principal">
          <tr><td><a id="add-campaign-1" href="#" title="<?php echo language::translate('showcase_title_row', 'Row'); ?>"><?php echo functions::form_draw_button('save_row', language::translate('showcase_add_row', 'Add Row'), 'submit', '', 'save'); ?></a></td></tr>
          <?php if (!empty($_POST['showcases'])) foreach (array_keys($_POST['showcases']) as $number=>$key) { ?>  
          <tr>
            <td>
            <strong style="font-size: 20px"><?php echo language::translate('showcase_title_row', 'Row').' '.($number+1); ?></strong>   <a class="move-up" href="#" title="<?php echo language::translate('text_move_up', 'Move up'); ?>"><?php echo functions::draw_fonticon('fa-arrow-circle-up fa-lg', 'style="color: #3399cc;"'); ?></a> <a class="move-down" href="#" title="<?php echo language::translate('text_move_down', 'Move down'); ?>"><?php echo functions::draw_fonticon('fa-arrow-circle-down fa-lg', 'style="color: #3399cc;"'); ?></a> <a class="remove-showcase_row" href="#" title="<?php echo language::translate('title_remove', 'Remove'); ?>"><?php echo functions::draw_fonticon('fa-times-circle fa-lg', 'style="color: #cc3333;"'); ?></a><br><br>            
              <?php echo functions::form_draw_hidden_field('showcases['.$key.'][id]', true) ?>
              <table id="table_showcase_secundary_<?php echo $key?>"> 
              <?php foreach(array_keys($_POST['showcases'][$key]['showcase_columns']) as $column_key){ ?>  
              <tr><td>
                <div class="content">
                <?php echo functions::form_draw_hidden_field('showcases['.$key.'][showcase_columns]['.$column_key.'][id]', true) ?>
                <strong><?php echo language::translate('showcase_title_column', 'Column'); ?><strong>    
                <a class="move-up" href="#" title="<?php echo language::translate('text_move_up', 'Move up'); ?>"><?php echo functions::draw_fonticon('fa-arrow-circle-up fa-lg', 'style="color: #3399cc;"'); ?></a> <a class="move-down" href="#" title="<?php echo language::translate('text_move_down', 'Move down'); ?>"><?php echo functions::draw_fonticon('fa-arrow-circle-down fa-lg', 'style="color: #3399cc;"'); ?></a> <a class="remove_column_<?php echo $key ?>" href="#" title="<?php echo language::translate('title_remove', 'Remove'); ?>"><?php echo functions::draw_fonticon('fa-times-circle fa-lg', 'style="color: #cc3333;"'); ?></a><br><br>
                 <strong><?php echo language::translate('showcase_column_title', 'Title'); ?></strong><br />
                 <?php echo functions::form_draw_text_field('showcases['.$key.'][showcase_columns]['.$column_key.'][title]', true); ?><br>
                   <strong><?php echo language::translate('showcase_column_subtitule', 'Subtitle'); ?></strong><br />
                 <?php echo functions::form_draw_textarea('showcases['.$key.'][showcase_columns]['.$column_key.'][subtitle]', true); ?><br>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <strong><?php echo language::translate('showcase_column_alignment', 'Text Alignment'); ?></strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                     <strong><?php echo language::translate('showcase_column_scheme', 'Scheme'); ?></strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                     <strong><?php echo language::translate('showcase_column_color', 'Color background'); ?></strong><br>
                     <?php echo functions::form_draw_select_field('showcases['.$key.'][showcase_columns]['.$column_key.'][align_text]',$options_alignment,true,false,'data-size="medium"'); ?>
                     <?php echo functions::form_draw_select_field('showcases['.$key.'][showcase_columns]['.$column_key.'][scheme_color]',$options_scheme,true,false,'data-size="medium"'); ?>
                 <?php echo functions::form_draw_text_field('showcases['.$key.'][showcase_columns]['.$column_key.'][background_color]',true, 'class="jscolor"'); ?><br><br>
                 <div id="image-<?php echo $_POST['showcases'][$key]['showcase_columns'][$column_key]['id']; ?>">
                 <?php if(!empty($_POST['showcases'][$key]['showcase_columns'][$column_key]['image'])){ 
                    echo $_POST['showcases'][$key]['showcase_columns'][$column_key]['image']; ?>
                  <a class="remove_image" onclick="remove_img('<?php echo $_POST['showcases'][$key]['showcase_columns'][$column_key]['id']; ?>'); return false;" href="#" title="<?php echo language::translate('title_remove', 'Remove'); ?>"><?php echo functions::draw_fonticon('fa-times-circle fa-lg', 'style="color: #cc3333;"'); ?></a><br><br>
                    <?php
                  } ?>
                 <img src="<?php echo functions::image_thumbnail(FS_DIR_HTTP_ROOT . WS_DIR_IMAGES . $_POST['showcases'][$key]['showcase_columns'][$column_key]['image'], 90, 65); ?>" align="left" style="margin: 5px;" /></div>
      <strong><?php echo language::translate('showcase_column_img-bf', 'Image background'); ?></strong><br>
                 <?php echo functions::form_draw_file_field('showcases['.$key.'][showcase_columns]['.$column_key.'][new_images_showcase]', 'data-size="large"'); ?><br>
                </div>
                </td></tr>
                <?php } ?> 
              </table>
              <a id="add-column-<?php echo $key ?>" href="#" title="<?php echo language::translate('showcase_add_column', 'Add Column'); ?>"><?php echo functions::draw_fonticon('fa-plus-circle', 'style="color: #66cc66;"'); ?></a>
            <hr>
            </td>
          </tr>
          <?php } ?>
    ...
    <script type="text/javascript">
          ...
          function remove_img(value){
            $('#table-showcase-principal').append('<input type="hidden" name="delete-archive[]" value="'+value+'">');
            $("#image-"+value).remove();
          }

        var new_principal_showcase_i = 1;

          $("body").on("click", "#add-campaign-1", function(event) {
            event.preventDefault();
            var output = '<?php echo functions::form_draw_hidden_field('showcases[new_principal_showcase_i][id]', true) ?>'
                +'<?php echo functions::form_draw_hidden_field('showcases[new_principal_showcase_i][showcase_columns][new_principal_showcase_i][id]', true) ?>';  
           while ($("input[name='showcases[new_"+new_principal_showcase_i+"]']").length) new_principal_showcase_i++;
            output = output.replace(/new_principal_showcase_i/g, 'new_' + new_principal_showcase_i);
            $("#table-showcase-principal tr:first").before(output);
            new_principal_showcase_i++;
            var input = $("<input>").attr("type", "hidden").attr("name", "save").val("save");
            $('form#catalogProduct').append($(input));
            var input_re = $("<input>").attr("type", "hidden").attr("name", "redirect").val("true");
            $('form#catalogProduct').append($(input_re));
            $('form#catalogProduct').submit();
          });

          function add_div(){
            new_principal_showcase_i--;
            var output1 = '<?php echo functions::general_escape_js(functions::form_draw_hidden_field('showcases[new_principal_showcase_i][showcase_columns][new_principal_showcase_i][id]', true)) ?>';
           while ($("input[name='showcases[new_"+new_principal_showcase_i+"]']").length) new_principal_showcase_i++;
            output1 = output1.replace(/new_principal_showcase_i/g, 'new_' + new_principal_showcase_i);
            $("#table_showcase_secundary_"+new_principal_showcase_i+" tr:last").before(output1);
            new_principal_showcase_i++;
            var input = $("<input>").attr("type", "hidden").attr("name", "save").val("save");
            $('form#catalogProduct').append($(input));
            var input_re = $("<input>").attr("type", "hidden").attr("name", "redirect").val("true");
          ...
          $("body").on("click", ".remove-showcase_row", function(event) {
            event.preventDefault();
            $(this).closest('tr').remove();
          });

          $("#table-showcase-principal").on("click", ".move-up, .move-down", function(event) {
                  event.preventDefault();
                  var row = $(this).closest("tr");
          ...
          <?php if (!empty($_POST['showcases'])) foreach (array_keys($_POST['showcases']) as $number=>$key) { ?>
          $("#table-showcase-secundary-<?php echo $key ?>").on("click", ".move-up, .move-down", function(event) {
                  event.preventDefault();
                  var row = $(this).closest("tr");
                ...
                $("body").on("click", ".remove_column_<?php echo $key ?>", function(event) {
                  event.preventDefault();
                  $(this).closest('tr').remove();
                });

        var new_showcase_i = 1;
          $("body").on("click", "#add-column-<?php echo $key ?>", function(event) {
            event.preventDefault();
            var output = '<?php echo functions::general_escape_js(functions::form_draw_hidden_field('showcases['.$key.'][showcase_columns][new_showcase_i][id]', true)) ?>';
           while ($("input[name='showcases[new_"+new_showcase_i+"]']").length) new_showcase_i++;
            output = output.replace(/new_showcase_i/g, 'new_' + new_showcase_i);
            $("#table_showcase_secundary_<?php echo $key?> tr:last").after(output);
            new_showcase_i++;
             var input = $("<input>").attr("type", "hidden").attr("name", "save").val("save");
            $('form#catalogProduct').append($(input));
            var input_re = $("<input>").attr("type", "hidden").attr("name", "redirect").val("true");
