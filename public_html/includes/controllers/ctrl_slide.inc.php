<?php
  ...
  class ctrl_slide {
    ...
    public function save() {
      ...
      database::query(
        ...
        set
          ...
          ". (!empty($this->data['image_mobile']) ? "image_mobile = '" . database::input($this->data['image_mobile']) . "'," : '') ."
    ...
    public function save_image_mobile($file) {
      
      if (!empty($this->data['image_mobile'])) {
        if (is_file(FS_DIR_HTTP_ROOT . WS_DIR_IMAGES . basename($this->data['image_mobile']))) {
          unlink(FS_DIR_HTTP_ROOT . WS_DIR_IMAGES . basename($this->data['image_mobile']));
        }
        $this->data['image_mobile'] = '';
      }
      ...
      $filename = 'slides/mobile-' . functions::general_path_friendly($this->data['id'] .'-'. $this->data['name'], settings::get('store_language_code')) .'.'. $image->type();
      ...
      $this->data['image_mobile'] = $filename;
      $this->save();
    }
